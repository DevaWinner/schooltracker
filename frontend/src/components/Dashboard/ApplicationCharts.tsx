...
export default function ApplicationCharts({ applications }: ApplicationChartsProps) {
  ...
  const statusChartData = useMemo(() => {
    ...
    const getStatusColor = (status: string) => {
      switch (status) {
        case "Draft": return 'rgba(156, 163, 175, 0.7)';
        case "In Progress": return 'rgba(59, 130, 246, 0.7)';
        case "Pending": return 'rgba(245, 158, 11, 0.7)';
        case "Accepted": return 'rgba(16, 185, 129, 0.7)';
        case "Rejected": return 'rgba(239, 68, 68, 0.7)';
        case "Deferred": return 'rgba(249, 115, 22, 0.7)';
        case "Withdrawn": return 'rgba(107, 114, 128, 0.7)';
        default: return 'rgba(209, 213, 219, 0.7)';
      }
    };
  ...
  const degreeChartData = useMemo(() => {
    ...
    applications.forEach(app => {
      if (app.degree_type) {
        degreeCounts[app.degree_type] = (degreeCounts[app.degree_type] || 0) + 1;
      }
    });
    ...
    const backgroundColor = labels.map(label => degreeColors[label] || 'rgba(209, 213, 219, 0.7)');
  ...
  const countryChartData = useMemo(() => {
    ...
    applications.forEach(app => {
      const country = app.institution_country || app.institution_details?.country || 'Unknown';
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-white/[0.05] dark:bg-white/[0.03]">
        <h3 className="mb-4 text-lg font-medium text-gray-800 dark:text-white/90">Status Distribution</h3>
        <div className="h-64">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-white/[0.05] dark:bg-white/[0.03]">
        <h3 className="mb-4 text-lg font-medium text-gray-800 dark:text-white/90">Degree Types</h3>
        <div className="h-64">
          {applications.length > 0 ? (
            <Bar 
      ...
      {/* Timeline Chart */}
      <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-white/[0.05] dark:bg-white/[0.03]">
        <h3 className="mb-4 text-lg font-medium text-gray-800 dark:text-white/90">Application Timeline</h3>
        <div className="h-64">
          {applications.length > 0 ? (
      ...
      {/* Countries Chart */}
      <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-white/[0.05] dark:bg-white/[0.03]">
        <h3 className="mb-4 text-lg font-medium text-gray-800 dark:text-white/90">Top Countries</h3>
        <div className="h-64">
          {applications.length > 0 ? (
